      ...
      try {
        ...
        if (response.body && typeof response.body.getReader === 'function') {
          ...
          while (true) {
            ...
            assistantMsgDiv.innerHTML = marked.parse(partialText);
            chatBox.scrollTop = chatBox.scrollHeight;
          }

          session.messages.push({ role: 'assistant', content: partialText });
        } else {
          const data = await response.json();
          const reply = data.message || data.choices?.[0]?.message?.content || 'No response generated.';
          assistantMsgDiv.innerHTML = marked.parse(reply);
